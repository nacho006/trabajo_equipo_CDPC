...
function drawTree(x, y, len, angle) {
  // Dibujamos el tronco del árbol
  stroke(0);
  line(x, y, x + len * cos(angle), y + len * sin(angle));

  // Si la rama es lo suficientemente larga, dibujamos dos ramas más pequeñas en
  // los extremos del tronco del árbol
  if (len > 10) {
    drawTree(x + len * cos(angle), y + len * sin(angle), len * 0.8, angle + PI / 6);
    drawTree(x + len * cos(angle), y + len * sin(angle), len * 0.8, angle - PI / 6);
  }
}
